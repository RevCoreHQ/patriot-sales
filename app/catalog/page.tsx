...
const TIER_COLORS: Record<MaterialTier, string> = {
  ...
  best: 'bg-[#C62828]/15 text-[#C62828] border-[#C62828]/20',
};
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="flex flex-wrap gap-4 mb-6">
            ...
            {(['all', 'good', 'better', 'best'] as const).map(t => (
              <button
                ...
                className={cn(
                  ...
                  tier === t ? 'border-[#C62828]/50 bg-[#C62828]/10 text-[#C62828]' : 'border-c-border-inner text-neutral-500 hover:text-neutral-300'
                )}
              >
          ...
          <div className="flex gap-1 flex-wrap">
            <button
              onClick={() => setCat('all')}
              className={cn('h-11 px-4 rounded-xl text-sm font-medium border transition-all cursor-pointer', cat === 'all' ? 'border-[#C62828]/50 bg-[#C62828]/10 text-[#C62828]' : 'border-c-border-inner text-neutral-500 hover:text-neutral-300')}
            >All Categories</button>
            {categories.map(c => (
              <button
                key={c}
                onClick={() => setCat(c)}
                className={cn('h-11 px-4 rounded-xl text-sm font-medium border transition-all cursor-pointer', cat === c ? 'border-[#C62828]/50 bg-[#C62828]/10 text-[#C62828]' : 'border-c-border-inner text-neutral-500 hover:text-neutral-300')}
              >
                {CAT_LABELS[c]}
              </button>
            ))}
            ...
            {filtered.map(m => (
              <button
                ...
                className={cn(
                  ...
                  m.id === selected ? 'border-[#C62828]/50 ring-1 ring-[#C62828]/20' : 'border-c-border-inner hover:border-c-border-hover'
                )}
              >
                ...
                <div className="p-3 bg-c-card">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <span className="text-xs font-semibold text-[#C62828]">${m.pricePerSqFt + m.laborPerSqFt}/sf installed</span>
                  </div>
                </div>
              </button>
            ))}
                  ...
                  <div className="space-y-1">
                    {selectedMaterial.features.map(f => (
                      <div key={f} className="flex items-center gap-1.5 text-xs text-c-text-2">
                        <div className="w-1 h-1 rounded-full bg-[#C62828] shrink-0" />
                        {f}
                      </div>
                    ))}
                  </div>
