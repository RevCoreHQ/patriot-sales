            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2.5 mt-1">
                  ...
                  {!isExpired && daysLeft <= 10 && !isSigned && <span className="flex items-center gap-1 text-xs font-medium text-accent"><Clock className="w-3.5 h-3.5" />{daysLeft}d left</span>}
          ...
          <div className="grid grid-cols-5 gap-5">
            ...
            <div className="col-span-3 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: 'Quote Total', value: formatCurrency(quote.total), color: 'text-accent' },
                  { label: 'Valid Until', value: formatDateShort(quote.validUntil), color: isExpired ? 'text-red-400' : daysLeft <= 7 ? 'text-accent' : 'text-c-text' },
                ].map(s => (
              ...
              <div className="rounded-2xl border border-c-border bg-c-card overflow-hidden">
                ...
                <div className="border-t border-c-border-inner divide-y divide-c-border-inner">
                  ...
                  <div className="px-6 py-4 flex justify-between items-center">
                    ...
                    <span className="font-bold text-2xl text-accent">{formatCurrency(quote.total)}</span>
                  </div>
                </div>
              </div>
              ...
              {quote.internalNotes && (
                <div className="rounded-2xl border border-accent/15 p-6 bg-accent/5">
                  <div className="text-xs font-bold text-accent/60 uppercase tracking-widest mb-2">Internal Notes</div>
            ...
            <div className="col-span-2 space-y-4">
              ...
              <div className="rounded-2xl border border-c-border bg-c-card p-5 space-y-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {quote.projectTypes.map(pt => (
                    <span key={pt} className="text-[11px] bg-accent/10 text-accent border border-accent/20 px-2.5 py-1 rounded-full capitalize font-medium">
                      {pt.replace(/-/g, ' ')}
                    </span>
                  ))}
                </div>
                <div className="text-xs text-c-text-4 space-y-1">
                  ...
                  {quote.siteConditions.tearOff && <div className="text-accent/60 font-medium">Tear-off required{quote.siteConditions.layers && quote.siteConditions.layers > 1 ? ` (${quote.siteConditions.layers} layers)` : ''}</div>}
                </div>
              </div>
              ...
              {/* Financing */}
              <div className="rounded-2xl border border-accent/20 overflow-hidden" style={{ background: 'linear-gradient(160deg, rgba(198,40,40,0.07), rgba(198,40,40,0.03))' }}>
                <button onClick={() => setShowFinancing(!showFinancing)} className="w-full flex items-center justify-between px-5 py-4 text-left">
                  <div className="flex items-center gap-3">
                    <CreditCard className="w-4.5 h-4.5 text-accent/70" />
                    <div>
                      <div className="text-sm font-bold text-accent">Financing Available</div>
                      {financing && !showFinancing && <div className="text-xs text-c-text-3 mt-0.5">From {formatCurrency(financing.monthlyPayment)}/mo</div>}
                    </div>
                  </div>
                  {showFinancing ? <ChevronUp className="w-4 h-4 text-accent/50" /> : <ChevronDown className="w-4 h-4 text-accent/50" />}
                </button>
                {showFinancing && (
                  <div className="px-5 pb-5 border-t border-accent/15 pt-4 space-y-3">
                    <div className="grid grid-cols-4 gap-2">
                      {settings.financing.map((opt, i) => (
                        <button key={opt.id} onClick={() => setSelectedTerm(i)} className={`py-2.5 rounded-xl text-xs font-semibold text-center transition-all ${selectedTerm === i ? 'bg-accent text-black' : 'bg-c-elevated text-c-text-3 active:bg-c-tag'}`}>
                          {opt.label}
                        </button>
                      ))}
                    </div>
              ...
              {isSigned ? (
                ...
                <button onClick={() => setShowSignature(true)} className="w-full flex items-center justify-between px-5 py-5 rounded-2xl border border-c-border bg-c-card active:border-accent/30 active:bg-accent/5 transition-all group">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-c-elevated flex items-center justify-center group-active:bg-accent/10 transition-all">
                      <PenLine className="w-4.5 h-4.5 text-c-text-4 group-active:text-accent transition-colors" />
                    </div>
                  ...
                  <ChevronDown className="w-4 h-4 text-c-text-4 group-active:text-accent transition-colors rotate-[-90deg]" />
                </button>
              )}
            </div>
          </div>
