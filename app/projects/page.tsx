    ...
    <div>
      ...
      <div className="relative">
        <input ref={ref} type="date" value={value} onChange={e => onChange(e.target.value)}
          className="w-full bg-c-input border border-c-border-input rounded-xl pl-3 pr-9 py-2.5 text-sm text-c-text focus:outline-none focus:border-[#fb8e28]/60" />
        <button type="button" onClick={() => ref.current?.showPicker?.()} className="absolute right-3 top-1/2 -translate-y-1/2 text-c-text-4 active:text-[#fb8e28] transition-colors">
    ...
    <div className="h-full flex flex-col overflow-hidden">
            ...
            return (
              <div key={p.id} className="flex items-center flex-1">
                <button onClick={() => updatePhase(project.id, p.id)} title={p.label} className="relative flex flex-col items-center group flex-1">
                  <div className={`w-4 h-4 rounded-full border-2 transition-all ${done ? 'bg-[#fb8e28] border-[#fb8e28]' : active ? 'bg-[#fb8e28]/30 border-[#fb8e28] ring-2 ring-[#fb8e28]/20' : 'bg-transparent border-c-border-input group-active:border-c-border-hover'}`} />
                  <span className={`text-[11px] mt-1 font-medium whitespace-nowrap transition-colors ${done ? 'text-[#fb8e28]' : active ? 'text-[#fcad55]' : 'text-c-text-5 group-active:text-c-text-4'}`}>{p.short}</span>
                </button>
                {!isLast && <div className={`h-0.5 flex-1 mx-0.5 rounded transition-all ${done ? 'bg-[#fb8e28]' : 'bg-c-border-inner'}`} />}
              </div>
            );
          })}
        </div>
      ...
      <div className="px-6 pb-4 grid grid-cols-3 gap-3 shrink-0">
        <div className="bg-c-surface border border-c-border-inner rounded-2xl p-4">
          ...
          <div className="mt-2 h-1.5 rounded-full bg-c-border-inner"><div className="h-full rounded-full bg-[#fb8e28] transition-all" style={{ width: `${pct}%` }} /></div>
        ...
        <div className="bg-c-surface border border-c-border-inner rounded-2xl p-4">
          ...
          <div className="mt-2 h-1.5 rounded-full bg-c-border-inner"><div className={`h-full rounded-full transition-all ${closeoutDone === closeoutTotal ? 'bg-emerald-500' : 'bg-[#fb8e28]'}`} style={{ width: `${closeoutTotal > 0 ? (closeoutDone / closeoutTotal) * 100 : 0}%` }} /></div>
      ...
      <div className="px-6 pb-3 flex items-center gap-1 border-t border-c-border-inner pt-3 shrink-0">
        ...
        <button onClick={() => setEditing(!editing)} className={`flex items-center gap-1.5 h-12 px-4 rounded-2xl text-sm font-medium transition-all ${editing ? 'bg-[#fb8e28]/15 text-[#fb8e28]' : 'text-c-text-4 active:text-c-text-2'}`}>
        ...
        {project.phase !== 'delivered' && (
          <button onClick={() => { if (project.phase === 'finishing' && !canDeliver) { alert('Complete all closeout items first.'); setTab('closeout'); return; } const idx = phaseIndex(project.phase); if (idx < PHASES.length - 1) updatePhase(project.id, PHASES[idx + 1].id); }}
            className="flex items-center gap-1.5 h-12 px-5 rounded-2xl text-sm font-semibold bg-[#fb8e28]/15 text-[#fb8e28] active:bg-[#fb8e28]/25 transition-all">
        ...
        {editing && (
          <div className="bg-c-surface border border-c-border-inner rounded-2xl p-5 mb-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              {payments.length === 0 && <div><label className="text-xs text-c-text-3 mb-1.5 block">Cash Collected ($)</label><input type="number" value={cash} onChange={e => setCash(e.target.value)} className="w-full bg-c-input border border-c-border-input rounded-xl px-3 py-2.5 text-sm text-c-text focus:outline-none focus:border-[#fb8e28]/60" /></div>}
              <div className={payments.length === 0 ? '' : 'col-span-2'}><label className="text-xs text-c-text-4 mb-1.5 flex items-center gap-1"><Link2 className="w-3 h-3" /> GHL Contact ID</label><input type="text" value={ghlId} onChange={e => setGhlId(e.target.value)} className="w-full bg-c-input border border-c-border-input rounded-xl px-3 py-2.5 text-sm text-c-text font-mono focus:outline-none focus:border-[#fb8e28]/60" /></div>
            </div>
            <div className="flex gap-2"><button onClick={saveEdits} className="h-12 px-6 bg-[#fb8e28] text-black text-sm font-bold rounded-2xl active:bg-[#fb8e28] active:scale-[0.97] transition-all">Save</button><button onClick={() => setEditing(false)} className="h-12 px-5 bg-c-elevated text-c-text-3 text-sm rounded-2xl active:bg-c-surface transition-all">Cancel</button></div>
          </div>
        )}
          ...
          <div>
            {project.updates.length === 0 ? <div className="text-xs text-c-text-4 py-3">No updates yet.</div> : (
              <div className="space-y-2.5 max-h-64 overflow-y-auto">{project.updates.map(u => (
                <div key={u.id} className="flex items-start gap-3"><div className="w-1.5 h-1.5 rounded-full bg-[#fb8e28]/50 mt-1.5 shrink-0" /><div><div className="text-sm text-c-text-2">{u.note}</div><div className="text-[11px] text-c-text-4 mt-0.5">{formatDateShort(u.date)} · <span className="capitalize">{u.phase.replace(/-/g, ' ')}</span></div></div></div>
              ))}</div>
            )}
            {project.phase !== 'delivered' && (
              <div className="flex gap-2 mt-4 border-t border-c-border-inner pt-4">
                <input value={noteText} onChange={e => setNoteText(e.target.value)} onKeyDown={e => e.key === 'Enter' && submitNote()} placeholder="Add a project update note..." className="flex-1 h-12 bg-c-surface border border-c-border-inner rounded-2xl px-4 text-sm text-c-text placeholder:text-c-text-4 focus:outline-none focus:border-[#fb8e28]/50" />
                <button onClick={submitNote} className="h-12 px-6 bg-[#fb8e28] text-black text-sm font-bold rounded-2xl active:bg-[#fb8e28] active:scale-[0.97] transition-all">Post</button>
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'todos' && (
          <div>
            ...
            <div className="flex gap-2 border-t border-c-border-inner pt-4">
              <input value={todoText} onChange={e => setTodoText(e.target.value)} onKeyDown={e => e.key === 'Enter' && submitTodo()} placeholder="Add a task..." className="flex-1 h-12 bg-c-surface border border-c-border-inner rounded-2xl px-4 text-sm text-c-text placeholder:text-c-text-4 focus:outline-none focus:border-[#fb8e28]/50" />
              <button onClick={submitTodo} className="h-12 w-12 flex items-center justify-center bg-[#fb8e28] text-black font-bold rounded-2xl active:bg-[#fb8e28] active:scale-[0.97] transition-all"><Plus className="w-4 h-4" /></button>
            </div>
          </div>
        )}
          ...
          <div>
            <div className="bg-c-surface border border-c-border-inner rounded-2xl p-5 mb-4">
              ...
              <div className="mb-3"><label className="text-[10px] text-c-text-4 mb-1.5 block">Payment Method</label><div className="grid grid-cols-3 gap-2">{PAYMENT_METHODS.map(m => (
                <button key={m.value} onClick={() => setPayMethod(m.value)} className={`flex items-center gap-2 h-12 px-3 rounded-2xl text-xs font-medium transition-all border ${payMethod === m.value ? 'bg-[#fb8e28]/15 border-[#fb8e28]/40 text-[#fcad55]' : 'bg-c-input border-c-border-input text-c-text-3 active:border-c-border-hover'}`}><span>{m.icon}</span>{m.label}</button>
              ))}</div></div>
              <div className="mb-3"><label className="text-[10px] text-c-text-4 mb-1.5 block">Note (optional)</label><input type="text" value={payNote} onChange={e => setPayNote(e.target.value)} onKeyDown={e => e.key === 'Enter' && submitPayment()} placeholder="e.g. check #1042" className="w-full h-12 bg-c-input border border-c-border-input rounded-2xl px-4 text-sm text-c-text placeholder:text-c-text-4 focus:outline-none focus:border-[#fb8e28]/60" /></div>
              <button onClick={submitPayment} disabled={!payAmount || parseFloat(payAmount) <= 0} className="w-full h-14 bg-[#fb8e28] disabled:opacity-40 disabled:cursor-not-allowed text-black text-base font-bold rounded-2xl active:bg-[#fb8e28] active:scale-[0.98] transition-all flex items-center justify-center gap-2"><Banknote className="w-4 h-4" />Record Payment</button>
            </div>
            {payments.length > 0 && (
              <>
                ...
                <div className="space-y-2 max-h-56 overflow-y-auto">{[...payments].reverse().map(tx => {
                  ...
                  return (
                    <div key={tx.id} className="flex items-center gap-3 bg-c-surface border border-c-border-inner rounded-2xl px-4 py-3 group">
                      ...
                      <div className="flex-1 min-w-0"><div className="flex items-center gap-2"><span className="text-sm font-bold text-c-text">{formatCurrency(tx.amount)}</span><span className="text-xs px-2 py-0.5 rounded-full bg-[#fb8e28]/10 text-[#fb8e28] font-medium">{typeLabel}</span><span className="text-xs text-c-text-4">{methodInfo?.label}</span></div>{tx.note && <div className="text-xs text-c-text-4 mt-0.5 truncate">{tx.note}</div>}<div className="text-xs text-c-text-5 mt-0.5">{formatDateShort(tx.date)}</div></div>
    ...
    <AppShell>
      {projects.length === 0 ? (
        ...
        <AnimatedPage className="flex h-full">
          ...
          <div className="w-[320px] shrink-0 border-r border-c-border-inner flex flex-col h-full overflow-hidden">
            <div className="p-5 shrink-0">
              <h1 className="text-xl font-bold text-c-text mb-3">Jobs</h1>
              {/* Metrics */}
              <div className="grid grid-cols-2 gap-2 mb-3">
                {[
                  { label: 'Active', value: String(stats.active), color: 'text-[#fb8e28]' },
              ...
              <div className="flex flex-wrap gap-1.5">
                <button onClick={() => setFilter('all')} className={`h-10 px-3 text-xs font-semibold rounded-xl transition-all ${filter === 'all' ? 'bg-[#fb8e28]/12 text-[#fb8e28] border border-[#fb8e28]/25' : 'bg-c-card text-c-text-4 border border-c-border active:bg-c-surface'}`}>All ({projects.length})</button>
                {PHASES.slice(0, -1).map(p => {
                  const count = projects.filter(proj => proj.phase === p.id).length;
                  if (count === 0) return null;
                  return <button key={p.id} onClick={() => setFilter(p.id)} className={`h-10 px-3 text-xs font-semibold rounded-xl transition-all ${filter === p.id ? 'bg-[#fb8e28]/12 text-[#fb8e28] border border-[#fb8e28]/25' : 'bg-c-card text-c-text-4 border border-c-border active:bg-c-surface'}`}>{p.short} ({count})</button>;
                })}
              </div>
            </div>
            {/* Project list */}
              ...
              {filtered.map(p => {
                ...
                return (
                  <button key={p.id} onClick={() => setSelectedId(p.id)} className={`w-full text-left px-5 py-4 border-b border-c-border-inner transition-colors ${isSelected ? 'bg-c-surface' : 'active:bg-c-surface/50'}`}>
                    ...
                    <div className="mt-1.5 h-1 rounded-full bg-c-border-inner"><div className="h-full rounded-full bg-[#fb8e28] transition-all" style={{ width: `${pct}%` }} /></div>
                  </button>
                );
              })}
            </div>
