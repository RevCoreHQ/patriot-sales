...
const TIER_COLORS: Record<MaterialTier, string> = {
  ...
  best: 'text-[#fb8e28] border-[#fb8e28]/30',
};
        ...
        {(['all', 'good', 'better', 'best'] as const).map(t => (
          <button key={t} type="button" onClick={() => setTier(t)}
            className={cn('px-3 py-1 rounded-full text-xs font-medium border transition-all capitalize cursor-pointer',
              tier === t ? 'bg-[#fb8e28]/15 border-[#fb8e28]/40 text-[#fb8e28]' : 'border-c-border-inner text-neutral-500 hover:text-neutral-300'
            )}>
          ...
          return (
            <button key={m.id} type="button" onClick={() => onSelect(m)}
              className={cn('flex items-center gap-3 p-3 rounded-lg border text-left transition-all cursor-pointer',
                selected === m.id ? 'border-[#fb8e28]/50 bg-[#fb8e28]/8' : 'border-c-border-inner bg-c-surface hover:border-c-border-hover'
              )}>
              <MaterialSwatch category={m.category} tier={m.tier} name={m.name} className="w-10 h-10 rounded-md shrink-0" />
              <div className="min-w-0 flex-1">
                <div className={cn('text-xs font-medium truncate', selected === m.id ? 'text-[#fb8e28]' : 'text-c-text')}>{m.name}</div>
              ...
              <div className="text-right shrink-0">
                <div className={cn('text-xs font-semibold', isGlobalOverride ? 'text-[#fb8e28]' : 'text-neutral-300')}>
                  ${effectiveTotal}/sf
                </div>
                {isGlobalOverride && <div className="text-[9px] text-[#fb8e28]/60">custom</div>}
              </div>
            </button>
          );
        })}
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        {materialSelections.map((sel, idx) => {
          ...
          return (
            <div key={idx} className={cn('border rounded-xl overflow-hidden', hasCustom ? 'border-[#fb8e28]/30' : 'border-c-border-inner')}>
              <div className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-c-elevated transition-colors"
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {hasCustom && <span className="text-[10px] bg-[#fb8e28]/15 text-[#fb8e28] px-1.5 py-0.5 rounded-full">custom price</span>}
                  </div>
                      ...
                      <div>
                        ...
                        <div className="text-[11px] text-neutral-600 mt-0.5">
                          ...
                          {(globalMat?.pricePerSqFt !== undefined || globalMat?.laborPerSqFt !== undefined) && (
                            <span className="text-[#fb8e28]/70 ml-1">
                      ...
                      <div className="relative">
                        ...
                        <input type="number" min="0" step="0.5"
                          ...
                          className={cn('w-full pl-7 pr-3 py-2.5 rounded-lg bg-c-input border text-sm text-c-text focus:outline-none focus:ring-1 focus:ring-[#fb8e28]/30',
                            sel.customPricePerSqFt !== undefined ? 'border-[#fb8e28]/40 bg-[#fb8e28]/5' : 'border-c-border-input'
                          )} />
                      </div>
                      <div className="relative">
                        ...
                        <input type="number" min="0" step="0.5"
                          ...
                          className={cn('w-full pl-7 pr-3 py-2.5 rounded-lg bg-c-input border text-sm text-c-text focus:outline-none focus:ring-1 focus:ring-[#fb8e28]/30',
                            sel.customLaborPerSqFt !== undefined ? 'border-[#fb8e28]/40 bg-[#fb8e28]/5' : 'border-c-border-input'
                          )} />
                      </div>
                    </div>
                  </div>
